...
import 'package:flutter/material.dart' show AlertDialog, BuildContext, FlatButton, Navigator, Text, TextButton, Widget, showDialog;
...
class FlutterCallkeep extends EventManager {
  ...
  Future<void> setup(BuildContext? context, Map<String, dynamic> options, {bool backgroundMode = false}) async {
    _context = context;
    if (!isIOS) {
  ...
  Future<void> displayIncomingCall(String uuid, String handle,
      {String localizedCallerName = '', String handleType = 'number', bool hasVideo = false}) async {
    if (!isIOS) {
      await _channel
          .invokeMethod<void>('displayIncomingCall', <String, dynamic>{'uuid': uuid, 'handle': handle, 'localizedCallerName': localizedCallerName});
      return;
    }
  ...
  Future<void> startCall(String uuid, String number, String callerName, {String handleType = 'number', bool hasVideo = false}) async {
    ...
    await _channel.invokeMethod<void>(
        'startCall', <String, dynamic>{'uuid': uuid, 'number': number, 'callerName': callerName, 'handleType': handleType, 'hasVideo': hasVideo});
  }
  ...
  Future<void> reportEndCallWithUUID(String uuid, int reason) async =>
      await _channel.invokeMethod<void>('reportEndCallWithUUID', <String, dynamic>{'uuid': uuid, 'reason': reason});
  ...
  Future<bool> isCalling() async {
    if (isIOS) {
      return false;
    }

    var resp = await _channel.invokeMethod('isCalling', <String, dynamic>{});
    if (resp != null) {
      return resp;
    }
    return false;
  ...
  Future<void> updateDisplay(String uuid, {required String displayName, required String handle}) async =>
  ...
  Future<bool> _alert(Map<String, dynamic> options, bool? showAccountAlert) async {
    ...
    var resp = await _showAlertDialog(_context!, options['alertTitle'] as String, options['alertDescription'] as String,
        options['cancelButton'] as String, options['okButton'] as String);
    if (resp != null) {
      return resp;
    }
    return false;
  ...
  Future<bool?> _showAlertDialog(BuildContext context, String? alertTitle, String? alertDescription, String? cancelButton, String? okButton) async {
